...
$.fn.cycle = function(options, arg2) {
	...
	return this.each(function() {
		...
		var arrHeightliNomal = $('#li_height_item_'+opts.divId);
            var maxHeightliNomal = 0;
            $.each(arrHeightliNomal,function(key, val){
                if(maxHeightliNomal<$(val).height()){
                    maxHeightliNomal = $(val).height(); 
                }
            });
            opts2.topScroll = maxHeightliNomal;
...
function buildButton(opts){
	var buttonNumber = $('.image_button_number_'+opts.divId).children();
    ...
    $.each(buttonNumber , function(key, val){
		//if($(val).attr('class')!='button_img_selected')
		$(val).bind('click', function(){return advanceClick(opts,opts.rev?-1:1,val.value)});	
	});
}
...
function resetButton(opts,index){
    var buttonNumber = $('.image_button_number_'+opts.divId).children();
	var buttons = $('.image_button_'+opts.divId).children();
    if(opts.theme == 'theme1' || opts.theme == 'theme2' || opts.theme == 'theme3' || opts.theme == 'theme4'){
        var buttonsSelectedTheme = 'button_img_selected_theme5_';
        var buttonsTheme = 'button_img_theme5_';
    }
    else{
        var buttonsSelectedTheme = 'button_img_selected_';
        var buttonsTheme = 'button_img_';
    }
    var active = '';
    var noactive = '';
    //var startNormalScroll = 0;
    //var endNormalScroll = opts.num_item_per_page - 1;
    //var totalNormalItem = opts.totalItemScroll;
    var startNormalScroll = opts.startNormalScroll;
    var endNormalScroll = opts.endNormalScroll;
    
	$.each(buttons , function(key, val){					  
		if(($(val).attr('class')==buttonsSelectedTheme+opts.divId))
		{  
			$(val).removeClass(buttonsSelectedTheme+opts.divId);$(val).addClass(buttonsTheme+opts.divId);
            if(opts.theme == 'theme1' || opts.theme=='theme2' || opts.theme == 'theme3' || opts.theme=='theme4'){
                $("#caption_"+opts.divId+key).fadeOut(0);
		...
		{
           if(opts.theme == 'theme1' || opts.theme=='theme2' || opts.theme=='theme3' || opts.theme == 'theme4'){
                $("#caption_"+opts.divId+key).fadeIn(2500);
            }
           $(val).removeClass(buttonsTheme+opts.divId);$(val).addClass(buttonsSelectedTheme+opts.divId);			
		}
    ...
    if(index == 0){
        opts.startNormalScroll = 0;
        opts.endNormalScroll = opts.num_item_per_page-1;
        if(opts.theme == 'theme1' || opts.theme=='theme2' || opts.theme=='theme3' || opts.theme == 'theme4'){
          $(".cover_buttons_"+opts.divId).animate({left:0}, 500);  
          $(".cover_buttons_number_"+opts.divId).animate({left:0}, 500);  
        }
        $(".cover_buttons_"+opts.divId).animate({top:0}, 500);
    }
    else{
        if(index == opts.endNormalScroll && index < opts.totalItemScroll - 1){
            
            if(opts.theme == 'theme1' || opts.theme=='theme2' || opts.theme=='theme3' || opts.theme == 'theme4'){
              $(".cover_buttons_"+opts.divId).animate({left:'-='+opts.leftScroll}, 500);  
              $(".cover_buttons_number_"+opts.divId).animate({left:'-='+opts.leftScrollNumber}, 500);  
            }
            opts.startNormalScroll +=1;
            opts.endNormalScroll +=1;
        }
        if(index == opts.startNormalScroll){
            if(opts.theme == 'theme1' || opts.theme=='theme2' || opts.theme=='theme3' || opts.theme == 'theme4'){
              $(".cover_buttons_"+opts.divId).animate({left:'+='+opts.leftScroll}, 500);  
              $(".cover_buttons_number_"+opts.divId).animate({left:'+='+opts.leftScrollNumber}, 500);  
            }
    ...
    {   
        if(opts.theme == 'theme1' || opts.theme == 'theme2' || opts.theme=='theme3' || opts.theme == 'theme4'){
            $(".cover_buttons_"+opts.divId).animate({left:"-="+((opts.totalItemScroll - (opts.num_item_per_page))* opts.leftScroll) }); 
            $(".cover_buttons_number_"+opts.divId).animate({left:"-="+((opts.totalItemScroll - (opts.num_item_per_page))* opts.leftScrollNumber) }); 
        }
        opts.startNormalScroll = (opts.totalItemScroll - 1) - (opts.num_item_per_page-1);
        opts.endNormalScroll = opts.totalItemScroll - 1;
    }
...
function buildOptions($cont, $slides, els, options, o) {
	...
	if (opts.pause)  
    {
        $('#caption_' + opts.divId).hover(function () {
        cont.cyclePause++;
        }, function () {
            cont.cyclePause--;
        });
        $('.button-next' + opts.divId).hover(function () {
        cont.cyclePause++;
        }, function () {
            cont.cyclePause--;
        });
        $('.button-prev' + opts.divId).hover(function () {
        cont.cyclePause++;
        }, function () {
            cont.cyclePause--;
        });
...
function go(els, opts, manual, fwd) {
	...
	var changed = false;
	if ((manual || !p.cyclePause) && (opts.nextSlide != opts.currSlide)) {//	alert('34');	
		changed = true;
		var fx = opts.fx;
        ...
        /*if(opts.theme=='theme1_' || opts.theme=='theme2_'){
            resetButton(opts, opts.nextSlide);  
        }
        */
	}
